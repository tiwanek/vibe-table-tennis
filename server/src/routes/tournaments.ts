    ...
    try {
      ...
      if (tournament.type === 'SWISS') {
        ...
        const eliminationMatches = allMatches.filter(
          (m) => m.tournamentStage === 'SEMIFINAL' || m.tournamentStage === 'FINAL'
        )
        ...
        if (eliminationMatches.length === 0) {
          ...
          if (pendingGroupMatches.length > 0) {
            throw new AppError(
              'All group stage matches must be confirmed before advancing to elimination',
              400
            )
          }
          ...
          if (pendingElimination.length > 0) {
            throw new AppError(
              'All current elimination matches must be confirmed before advancing',
              400
            )
          }
